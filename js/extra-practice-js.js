...
function totalAmount(item1,price1,item2,price2,item3,price3) {
	...
	console.log(((item1 * price1)+(item2*price2)+(item3*price3))-(((item1*price1)+(item2*price2)+(item3*price3))*0.20));
	if (item1&&item2&&item3) {
		...
		alert("Your customer loyalty savings this week is: $"+(((item1*price1)+(item2*price2)+(item3*price3))*0.20).toFixed(2)
...
function smeltingTime(metalOnHand,metalToBuy,metalToSmelt) {
	if (metalToSmelt>=40) {
		...
		console.log(Math.floor(Number(metalToSmelt/40)).toFixed(0)+" Ingots are sold for $"+((Math.floor(Number(metalToSmelt/40)))              *225).toFixed(2)+".");
